...
export default function WaterQualityScreen() {
  ...
  const modelPredict = async ({ lat, lon }) => {
    return { weight: Math.random() * 50 + 5, source: "predicted", waste: "No data" };
  };
  ...
  useEffect(() => {
    ...
    // Load globe data (includes waste)
  ...
  const handleSearch = async () => {
    ...
    // Find nearest globe data (weight + waste)
    let globePoint = null;
    minDist = Infinity;
    globeData.forEach(p => {
      const dist = Math.sqrt((lat - p.Latitude) ** 2 + (lon - p.Longitude) ** 2);
    ...
    setResult({
      ...
      waste: globePoint.weight ? globePoint.weight.toFixed(2) : "No data",
